...
class IdentifierService:
    ...
    def update(
        ...
        if not identifier:
            return None
        old_type = identifier.type
        for key, value in data.model_dump(exclude_unset=True).items():
            setattr(identifier, key, value)
        if old_type == IdentifierType.UID and identifier.type != IdentifierType.UID:
            identifier.mifare_credential = None
        self.db.commit()
        self.db.refresh(identifier)
        return identifier
